...
from gen_env.configs.config import RLConfig, TrainAccelConfig, TrainConfig
from gen_env.envs.play_env import GenEnvParams
...
def make_train(config: TrainConfig, restored_ckpt, checkpoint_manager):
    ...
    def train(rng, config: TrainConfig):
        ...
        queued_state = GenEnvParams(ctrl_trgs=jnp.zeros(len(env.prob.stat_trgs)))
        ...
        queued_state_e = GenEnvParams(ctrl_trgs=jnp.zeros(len(env.prob.stat_trgs)))
...
def init_checkpointer(config: RLConfig):
    ...
    breakpoint()
    env, env_params = init_base_env(cfg=config)
    breakpoint()
    # env = FlattenObservationWrapper(env)
    env = LogWrapper(env)
    rng, _rng = jax.random.split(rng)
...
def main(config: TrainAccelConfig):
    breakpoint()
