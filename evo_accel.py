...
from gen_env.configs.config import TrainConfig
...
def apply_evo(rng, env: PlayEnv, ind: Individual, evo_state: EvoState, network_params, network,
    ...
    env_params = evo_state.env_params
    mutate_fn = jax.vmap(ind.mutate, in_axes=(0, 0, None))
    # Just mutating all elites once atm
    maps, ruless = mutate_fn(evo_rng, env_params, config)
    env_params = env_params.replace(maps=maps, rules=ruless)
 
    def eval_frzs(env_params, network_params):
        env_params: GenEnvParams = jnp.tile(
            frz_maps, (int(np.ceil(config.n_envs / env_params.rules[0])), 1, 1)
        )[:config.n_envs]
    ...
    fits, states = eval_params(env_params, network_params)    
    ...
    elite_params = jax.tree_map(lambda x: x[top_indices], frz_maps)
    ...
    return EvoState(top_fitness=top_fitnesses, env_params=top) # here do I need to init an empty one and evo_state.replace(top_fitness=top_fitnesses, frz_map=top) ?
    # return top
